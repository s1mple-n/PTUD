...
public class GDCapNhatGhiChu extends JDialog implements IDSBienGDCapNhatGhiChu {
    private static GDCapNhatGhiChu gdCapNhatGhiChu = null;
    private int viTriGhiChuCanCapNhat = 0;
    private GhiChu ghiChuCanCapNhat;

    private GDCapNhatGhiChu(){
    ...
    public static GDCapNhatGhiChu getGdThemGhiChu() {
        if (gdCapNhatGhiChu == null)
            gdCapNhatGhiChu = new GDCapNhatGhiChu();
        return gdCapNhatGhiChu;
    }
    ...
    private void dungPnlChuaBtn(){
        ...
        btnThoat.setMaximumSize(dimBtn);
        datHanhDongChoBtnThoat();
        pnlChuaBtn.add(btnThoat);

        pnlChuaBtn.add(Box.createHorizontalStrut(20));
        ...
        btnCapNhatGhiChu.setMaximumSize(dimBtn);
        datHanhDongChoBtnCapNhatGhiChu();
        pnlChuaBtn.add(btnCapNhatGhiChu);
    }
    ...
    private void datHanhDongChoBtnCapNhatGhiChu(){
        btnCapNhatGhiChu.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                thucHienThuTucThemGhiChuMoi();
            }
    ...
    private boolean kiemTraSuHopLeCuaHanThucHien(){
        ...
        if (!(
                hanThucHien.after(gioBatDauCa)
                        && hanThucHien.after(gioHienTai)
                        && hanThucHien.before(gioKetThucCa))
        ){
            tmpHanThucHien.getComponentTimeTextField().requestFocus();
            return false;
        }
        ...
        if (
            ...
            new Thread(() -> {
              CacHamDungSan.hienThiThongBaoKetQua(
                      ...
                      "Ghi chú đã được cập nhật thành công (^_^)"
              );
            }).start();
        }
        else{
    ...
    public void cungCapViTriVaGhiChuCanCapNhat(int viTri, GhiChu ghiChu){
        this.viTriGhiChuCanCapNhat = viTri;
        this.ghiChuCanCapNhat = ghiChu;

        CacHamDungSan.duaTxtVeTrangThaiSanSangHienThiThongTin(txtChuDe, ghiChu.getChuDe());
        CacHamDungSan.duaTxtVeTrangThaiSanSangHienThiThongTin(tmpHanThucHien.getComponentTimeTextField(), ghiChu.getHanThucHien().toLocalTime() + "");
        CacHamDungSan.duaTxaVeTrangThaiSanSangHienThiThongTin(txaNoiDungGhiChu, ghiChu.getNoiDungGhiChu());
    }
}